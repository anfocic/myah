...
| Eval harness | Deterministic grading of `run_agent` against canned tasks. Three dimensions: tool-use hygiene (trace assertions), content correctness (regex/substr), filesystem side effects (fs_file_*). Python task modules, per-task provider override, JSONL results. Second real caller of `run_agent` after the subagent tool — proves the signature is library-shaped. See `evals/` and §45 | Medium | Shipped (2 seed tasks; LLM-judge deferred) |
